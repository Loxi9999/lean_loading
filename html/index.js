...
function setBackgroundImageWithTransition() {
    $(".bg").css(
        "background-image",
        `linear-gradient(to bottom,rgba(0, 0, 0, 0.6),rgba(0, 0, 0, 0.6)), url("${getRandomBg()}.png");`
    );
}

setBackgroundImageWithTransition();
setInterval(setBackgroundImageWithTransition, 8000);
...
window.addEventListener("click", (event) => {
    if (musicStatus) {
        musicStatus = false;
        music.pause();
        $("#musicstatus").html("] Wznów muzyke");
    } else {
        musicStatus = true;
        music.play();
        $("#musicstatus").html("] Zatrzymaj muzyke");
    }
});
